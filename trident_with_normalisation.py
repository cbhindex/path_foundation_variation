...
Apply stain normalisation before Trident patch embedding extraction.

This script is a bridge around Trident that keeps Trident-compatible ``.h5``
outputs while inserting GPU-accelerated stain normalisation in the patch
pipeline.

Workflow per slide:
1. Load patch coordinates from ``*_patches.h5``.
2. Read image patches via cuCIM directly on GPU.
3. Apply stain normalisation (Vahadane/Macenko/Reinhard).
4. Run Trident encoder inference and save ``features`` + ``coords`` as ``.h5``.

Use ``--custom_list_of_wsis`` to process only selected slides from a CSV with a
single header column ``wsi``.
...
--coords_dir : str
    Directory containing ``*_patches.h5`` coordinate files.
--out_dir : str
    Output directory for generated ``.h5`` feature files.
--target_img_path : str
    Target image used to fit the stain normaliser.
--encoder_name : str
    Trident patch encoder name (for example ``uni_v2``).
--norm_method : {"vahadane", "macenko", "reinhard"}
    Stain normalisation method.
--batch_size : int
    Number of patches processed per batch.
--gpu : int
    CUDA device index.
--mag : int
    Desired magnification (for example ``20``).
--custom_list_of_wsis : str, optional
    CSV path with header ``wsi`` to restrict processing to selected files.
"""
...
def make_patch_loader(
    ...
    """
    Build a GPU patch loader backed by cuCIM for one slide.
    ...
    patch_size : int
        Patch edge length (pixels) at the selected level.
    device : torch.device
    ...
    -------
    callable
        Function ``load_xy(x, y) -> torch.Tensor`` returning a ``(C, H, W)``
        float tensor in ``[0, 1]`` on ``device``.
    """
...
def _collate_batch(batch):
    """
    Collate ``PatchDataset`` samples into batched patch tensors and coordinates.

    Parameters
    ----------
    batch : list[tuple[torch.Tensor, tuple[int, int]]]
        Sample list from ``PatchDataset``.

    Returns
    -------
    tuple[torch.Tensor, torch.Tensor]
        Batched patches ``(N, C, H, W)`` and coords ``(N, 2)``.
    """
...
def process_slide(
        ...
        level: int) -> None:
    """
    Process one slide: normalise patches, encode features, write Trident-style H5.
    ...
    coords_path : str
        Path to Trident coordinate H5 file (``*_patches.h5``).
    out_path : str
        Output H5 path for extracted features and coordinates.
    normalizer : torch_staintools.normalizer.NormalizerBuilder
        Fitted stain normaliser.
    patch_encoder : torch.nn.Module
        Trident patch encoder in eval mode.
    post_normalize : torchvision.transforms.Normalize
        Mean/std normalisation expected by the selected encoder.
    batch_size : int
        Number of patches per inference batch.
    device : torch.device
        Runtime device.
    level : int
        WSI pyramid level corresponding to desired magnification.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run stain-normalised patch embedding extraction with Trident."
    )
